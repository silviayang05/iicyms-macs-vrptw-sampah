...
class Node:
    def __init__(self, id: int, longitude: float, latitude: float, demand: float, ready_time: float, due_time: float, service_time: float, name: str):
        super()
        self.id = id
        ...
        with open(file_path, 'rt') as f:
            ...
            for line in f:
                if count == 5:
                    ...
                    parts = line.split()
                    node_list.append(Node(int(parts[0]), float(parts[1]), float(parts[2]), float(parts[3]), float(parts[4]), float(parts[5]), float(parts[6]), ' '.join(parts[7:])))
                count += 1
